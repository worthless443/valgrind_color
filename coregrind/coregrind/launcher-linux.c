...
int main(int argc, char** argv, char** envp)
{
   ...
   for (i = 1; i < argc; i++) {
	  ...
	  if(*argv[i]=='-') {
	  	for(_delim=0;*(argv[i] + _delim)!='=';++_delim) if(_delim>=0xff) break;
	  	memcpy(flag_buf,argv[i], _delim);
		if(_delim==0xff) {
			fprintf(stderr,"%s:option not found\n", argv[i]);
			return 1;
		}
	  }
	  if(0 == strcmp(flag_buf, "--color"))  {
		  if(0 == strcmp(argv[i] + _delim + 1, "auto")) {
			  COLOR_UMSG_ = 1;
		  }
		  else  {
			  fprintf(stderr, "valgrind::color: %s is either not a valid option or the option %s is beig worked on.\n", argv[i] + _delim + 1, argv[i] + _delim + 1);
			  return 1;
		  }
	  }
      if (0 == strcmp(argv[i], "--")) {
   ...
   int fd = open(_COLOR_CONFIG_FILE, O_WRONLY);
   if(COLOR_UMSG_==1) write(fd,"0", 1);
   else write(fd,"1",1);
   close(fd);
