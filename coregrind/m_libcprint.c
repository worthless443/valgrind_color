...
int read_file_asm3(char *fn) {
	...
	asm("mov %rdi, 0x1f(%rsp)");
	asm("mov %r14, 0x1c(%rsp)");
	asm("mov %rdx, 0x14(%rsp)");
	asm("mov %rsi, 0x28(%rsp)");
	asm("mov %r12, 0x30(%rsp)");
	 //asm("mov %%r11, %0" : "=r" (fn));
	...
	asm("mov %rax, %r12");
	  ...
	  asm("mov %rsp, %r14");
	  asm("lea -0xf(%r14), %r11");
	  asm("mov %r11, -0x20(%r14)");
	  asm("mov $0, %rax");
	  asm("mov %r12, %rdi");
	  asm("mov -0x20(%r14), %rsi");
	  asm("mov $1, %rdx");
	  asm("syscall");

	  asm("mov -0x20(%%r14), %0" : "=r" (fn));
	  int ret = *fn - '0',i;
	  for(i=0;*(fn + i) - '0'!=3;++i);

	  // IMPORTANT: restore the non-calle-reserved registers
	  	asm("mov 0x1f(%rsp), %rdi");
	  	asm("mov 0x28(%rsp), %rsi");
	  	asm("mov 0x14(%rsp), %rdx");
	  	asm("mov 0x1c(%rsp), %r14");
	  	asm("mov 0x30(%rsp), %r12");

	  return ret;
}
int read_file_asm_withname(const char *fn, char *buf) {
	...
	asm("mov %rax, %r12");
	  ...
	  asm("mov -0x20(%%r14), %0" : "=r" (buf));
	  int ret = *buf - '0',i;
	  for(i=0;(*(buf + i) - '0')!=4;++i) if(i>500) break;
...
static int read_file_asm2(char *fn) {
	asm("lea 0xe(%rbp), %r11\n\t"
		"movl $110, (%r11)\n\t"
		"movl $0x69, 1(%r11)\n\t"
		"movl $0x67, 2(%r11)\n\t"
...
static int read_file_asm(char *fn, char *buf) {
	int fd = 0;
	asm("lea 0xe(%rsp), %r11\n\t"
		"movl $110, (%r11)\n\t"
		"movl $0x69, 1(%r11)\n\t"
...
{ 
	//
   // open("test.c", 0); // look  what i had to do
   //int n = ead_file_asm3(&f);
   char buf[10];
   int n = read_file_asm_withname(_COLOR_CONFIG_FILE, buf);
   int times = 0;
